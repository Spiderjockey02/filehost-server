				...
				<div className="modal-content">
					...
					<div className="modal-body">
						<form onSubmit={createNewStorage} method="post">
							<InputField title='Name' name="name" errorMsg={errorMsg.startsWith('name') ? errorMsg : ''} onChange={(e) => setStorage((s) => ({ ...s, name: e.target.value }))} />

							<div className="mb-3">
								<label htmlFor="storageType" className="form-label">Type</label>
								<select className="form-select" id="storageType" required style={errorMsg.startsWith('type') ? { borderColor: 'red' } : {}}>
							...
							<Row>
								<Col md={6}>
									<InputField title='Latitude' name="Latitude" type="number" errorMsg={errorMsg.startsWith('latitude') ? errorMsg : ''} onChange={(e) => setStorage((s) => ({ ...s, latitude: e.target.value }))} />
								</Col>
								<Col md={6}>
									<InputField title='Longitude' name="Longitude" type="number" errorMsg={errorMsg.startsWith('longitude') ? errorMsg : ''} onChange={(e) => setStorage((s) => ({ ...s, longitude: e.target.value }))} />
								</Col>
							</Row>
							...
							<InputField title='Max Size (MB)' name="Max Size (MB)" type="number" errorMsg={errorMsg.startsWith('maxSize') ? errorMsg : ''} onChange={(e) => setStorage((s) => ({ ...s, maxSize: e.target.value }))} />
